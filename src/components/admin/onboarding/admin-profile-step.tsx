...
import { setupAdminProfileAction, type AdminProfileSetupState } from "@/app/[locale]/(auth)/sign-in/admin/onboarding/actions";
...
function SubmitButton() {
  ...
  const t = useTranslations("admin.onboarding.profile.actions");
  return (
    <button
      type="submit"
      disabled={pending}
...
export function AdminProfileStep({
  ...
  const t = useTranslations("admin.onboarding.profile");
  const tFields = useTranslations("admin.onboarding.profile.fields");
  const tFileStatus = useTranslations("admin.onboarding.profile.fileStatus");
  const tCrop = useTranslations("admin.onboarding.profile.crop");
  const tActions = useTranslations("admin.onboarding.profile.actions");
            ...
            <div className="flex flex-col items-center gap-4 sm:flex-row">
              ...
              <div className="flex-1">
                ...
                <p className="mt-1 text-xs text-white/70">
                  {draft.avatarFileName ?? (profile.avatar_url ? tFileStatus("existing") : tFileStatus("noFile"))}
                </p>
              </div>
            </div>
      ...
      {isAvatarCropOpen && avatarRawImage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4">
          <div className="w-full max-w-xl space-y-4">
            ...
            <div className="space-y-4">
              <div className="space-y-2">
                <label className="text-sm text-[var(--muted)]">{tCrop("zoom")}</label>
                <input
                  type="range"
                  min={1}
                  max={3}
